...
// Implementa Entrega2.buscarLibrosPorAutor()
...
fun buscarLibrosPorAutor(autor: String, libros: List<Libro>): Result<List<Libro>> {
    if (autor.isBlank()) {
        return Result.failure(Exception("El Apuntes.nombre del autor no puede estar vacío."))
    }

    // Filtra la Apuntes.lista y convierte Apuntes.a minúsculas para una búsqueda insensible Apuntes.a mayúsculas.
...
// Implementa Entrega2.buscarLibrosPorRangoAño() ← Usa filter con rango
...
// Implementa Entrega2.librosDisponibles
...
// Implementa Entrega2.calcularEstadisticas()
fun calcularEstadisticas(libros: List<Libro>): Result<String> {
    if (libros.isEmpty()) {
        return Result.failure(Exception("La Apuntes.lista de libros está vacía."))
    }
...
fun <T> manejarResultado(titulo: String, resultado: Result<T>) { // Imprime de forma ordenada cualquier Apuntes.resultado obtenido con Result.
    println("\n=== $titulo ===")
    if (resultado.isFailure) {
        ...
        println("ÉXITO:")
        // Si el Apuntes.resultado es una Apuntes.lista o un mapa, lo imprimimos de forma limpia
...
fun main() {
        ...
        // Búsqueda por Autor
        print("\nIntroduce el Apuntes.nombre del autor Apuntes.a buscar: ")
        ...
        // Búsqueda por Título
        print("\n Introduce una palabra clave del título Apuntes.a buscar: ")

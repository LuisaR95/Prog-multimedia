...
import Apuntes.validarEmail
import Apuntes.validarNombre
...
// Creación de listas
// Entrega2.listaContactos: almacena todos los contactos creados.
//Entrega2.siguienteId: aumenta cada vez que se crea un nuevo contacto.
...
//Validación Apuntes.nombre
//El Apuntes.nombre debe tener mínimo 3 letras.
fun validarNombre1(nombre: String): Result<String> =
    if (nombre.length < 3) Result.failure(Exception("El Apuntes.nombre debe tener al menos 3 caracteres"))
...
//Lo añade Apuntes.a la Apuntes.lista.
...
// Buscar por Apuntes.nombre
// Si el Apuntes.nombre está vacío → devuelve todos.
...
//Eliminar contacto
//removeIf elimina en base Apuntes.a condición.
//Si elimina → éxito.
    ...
    while (continuar) {
        ...
        when (opcion) {
            1 -> {
                ...
                crearContacto(n, t, e)
                    .onSuccess { println("Entrega2.Contacto creado (ID ${it.id})") }
                    .onFailure { println("${it.message}") }
            }

            2 -> {
                print("Buscar Apuntes.nombre: "); val n = readLine() ?: ""
            ...
            7 -> {
                print("ID: "); val id = readLine()?.toIntOrNull() ?: 0
                eliminarContacto(id)
                    .onSuccess { println(" Entrega2.Contacto eliminado") }
                    .onFailure { println(" ${it.message}") }
            }

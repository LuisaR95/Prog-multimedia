...
//DateTimeFormatter → da formato legible Apuntes.a las fechas.
data class Nota(
    val id: Int,
    var titulo: String,
...
fun main() {
    // Variables de estado
    //notas: Apuntes.lista donde se guardan todas las notas.
    ...
    //Crea una nueva Entrega2.Nota con ID autoincremental.
    //La agrega Apuntes.a la Apuntes.lista.
    fun crearNota() {
        println("\n--- Crear Nueva Entrega2.Nota ---")
        print("Título: ")
        ...
        notas.add(nuevaNota)
        println("✅ Entrega2.Nota ID ${nuevaNota.id} creada con éxito.")
    }
    ...
    //Pide un término.
    //Convierte todo Apuntes.a minúsculas.
    //Filtra si aparece en título o contenido.
    //Muestra los resultados formateados (usando map para Entrega2.convertir objetos en texto).
    fun buscarNotas() {
    ...
    // Modificar nota
    fun modificarNota() {
        print("\nIntroduce el ID de la nota Apuntes.a modificar: ")
        val id = readLine()?.toIntOrNull() ?: return

        val nota = obtenerNotaPorId(id)

        if (nota == null) {
            println("Entrega2.Nota con ID $id no encontrada.")
            return
        }

        println("Modificando Entrega2.Nota ID ${nota.id} (Título actual: ${nota.titulo})")
        ...
        println("✅ Entrega2.Nota ID ${nota.id} modificada con éxito.")
    }
    ...
    fun marcarImportante() {
        ...
        println("✅ Entrega2.Nota ID ${nota.id} $estado.")
    }
    ...
    //Muestra mensaje de confirmación
    fun eliminarNota() {
        print("\nIntroduce el ID de la nota Apuntes.a ELIMINAR: ")
        val id = readLine()?.toIntOrNull() ?: return

        val eliminada = notas.removeIf { it.id == id }

        if (eliminada) {
            println("Entrega2.Nota ID $id eliminada con éxito.")
        } else {
            println("Entrega2.Nota con ID $id no encontrada.")
        }
    }

    // Exportar notas Apuntes.a formato texto (Requisito: writeText)
    fun exportarNotas() {
        if (notas.isEmpty()) {
    ...
    while (ejecutando) {
        ...
        println("1. Crear Entrega2.Nota")
        println("2. Mostrar Todas las Notas (por fecha)")
        println("3. Buscar Notas (por título/contenido)")
        println("4. Modificar Entrega2.Nota")
        println("5. Marcar/Desmarcar como Importante")
        println("6. Eliminar Entrega2.Nota")
        println("7. Exportar Notas Apuntes.a Texto")
        println("0. Salir")
